    ...
    <main class="p-4 space-y-4">
      ...
      <div v-if="lease" class="bg-white rounded-2xl shadow p-4 space-y-1">
        ...
        <div class="flex justify-between items-center mt-2">
          <span class="inline-block text-xs px-2 py-1 rounded-full bg-green-100 text-green-700">{{ lease.status }}</span>
          <button @click="terminateLease" :disabled="terminating" class="text-xs text-red-500 px-2 py-1 disabled:opacity-50">
          ...
          :href="whatsappLink"
          target="_blank"
          class="block text-center mt-2 text-sm font-medium text-white bg-green-600 rounded-xl py-2"
        >
          💬 Send WhatsApp Reminder
        </a>
        <p v-else class="text-xs text-slate-400 mt-2">Add a phone number to enable WhatsApp reminders.</p>
      </div>
      ...
      <!-- Past leases -->
      <div v-if="pastLeases.length > 0" class="bg-white rounded-2xl shadow p-4 space-y-2">
        <h3 class="font-semibold text-slate-700">Lease History</h3>
        <div v-for="pl in pastLeases" :key="pl.id" class="flex justify-between items-center py-2 border-b last:border-0">
          <div>
            <p class="text-sm font-medium text-slate-700">{{ pl.units.unit_label }} — {{ pl.units.properties.name }}</p>
            <p class="text-xs text-slate-400">{{ pl.move_in_date }} → {{ pl.lease_expiry }}</p>
          </div>
          <span class="text-xs px-2 py-1 rounded-full bg-slate-100 text-slate-500">{{ pl.status }}</span>
        </div>
      </div>
...
const whatsappLink = computed(() => {
  if (!tenant.value?.phone || !lease.value) return '#'

  let phone = tenant.value.phone.replace(/[\s-]/g, '')
  if (phone.startsWith('0')) phone = '255' + phone.slice(1)
  if (phone.startsWith('+')) phone = phone.slice(1)

  const propertyLabel = `${lease.value.units.unit_label} — ${lease.value.units.properties.name}`
  const message = `Habari ${tenant.value.full_name}, kumbusho la kodi ya nyumba kwa ${propertyLabel}. Kiasi: TZS ${formatMoney(lease.value.monthly_rent)}. Kiasi kilichobaki: TZS ${formatMoney(outstanding.value)}. Asante.`
...
async function fetchPastLeases() {
  const { data } = await supabase
    .from('leases')
    .select('*, units(unit_label, properties(name))')
    .eq('tenant_id', tenantId)
    .neq('status', 'active')
    .order('lease_expiry', { ascending: false })
  pastLeases.value = data || []
}
...
async function terminateLease() {
  ...
  if (!confirm('End this lease? The unit will become vacant and you can assign a new tenant.')) return
  ...
  if (!leaseError) {
    await supabase.from('units').update({ is_occupied: false }).eq('id', lease.value.unit_id)
    lease.value = null
    await fetchPastLeases()
    await fetchVacantUnits()
    payments.value = []
  } else {
    alert(leaseError.message)
